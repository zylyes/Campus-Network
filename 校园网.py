...
from tkinter import messagebox, ttk  # 从tkinter导入messagebox和ttk，用于图形界面中的对话框和高级组件
...
from logging.handlers import TimedRotatingFileHandler  # 从logging.handlers导入TimedRotatingFileHandler，用于按时间轮转记录日志到文件
...
class PasswordFilter(logging.Filter):
    def filter(self, record):
        ...
        if 'user_password=' in message:  # 如果日志信息中包含'user_password='
            new_message = message.replace(message.split('user_password=')[1].split('&')[0], '********')  # 将密码部分替换为'********'
...
class CampusNetLoginApp:
    ...
    def __init__(self, master, show_ui=True):
        ...
        self.eye_open_icon = tk.PhotoImage(file="./ico/eye_open.png")  # 导入眼睛图标-打开状态
    ...
    def load_or_generate_key(self):
        ...
        if os.path.exists(key_file):  # 如果密钥文件已存在
            ...
            messagebox.showinfo("密钥生成", "新的加密密钥已生成并保存。")  # 弹出提示框显示密钥已生成
    ...
    def login(self):
        ...
        if self.validate_credentials(username, password):  # 调用验证函数检查用户名和密码是否为空
            # 启动后台线程执行登录操作，并传递用户名和密码
            login_thread = threading.Thread(target=self.perform_login, args=(username, password, False))
            login_thread.start()
        else:
            messagebox.showwarning("验证失败", "用户名或密码为空，请按要求填写。")  # 显示警告框，提示用户名或密码为空
    ...
    def decode_base64_message(self, b64message):
        """解码Base64消息"""
        try:
            return base64.b64decode(b64message).decode('utf-8')  # 尝试解码Base64消息并以utf-8编码返回
        except Exception as e:
            logging.error(f"Base64消息解码失败:{e}")  # 记录错误日志，提示Base64消息解码失败
            return None
        ...
        try:
            ...
            if result == "1":
                ...
                if "ldap auth error" in decoded_message:
                    ...
                    self.master.after(0, lambda: self.show_notification("登录失败", "未知错误，请去报告错误界面提交错误提示后重新尝试", self.config['icons']['unknown']))
                    # 如果是自动登录且登录失败，考虑显示UI或通知用户
                    if self.show_ui:  # 如果允许显示UI，则重启UI流程
                        self.master.after(0, self.setup_ui)
                        self.master.after(0, lambda: self.show_error_message("自动登录失败", "自动登录失败，未知错误，请去报告错误界面提交错误提示后重新尝试。"))
            else:
                # 记录不符合任一已定义条件的返回值
                ...
                # 尝试打开常见问题文档
                self.master.after(0, lambda: os.startfile(os.path.join(os.getcwd(), "常见问题.docx")))
                self.master.after(0, lambda: self.show_notification("登录失败", "无法解码消息，请去报告错误界面提交错误提示后重新尝试", self.config['icons']['unknown']))
                # 如果是自动登录且登录失败，考虑显示UI或通知用户
                if self.show_ui:  # 如果允许显示UI，则重启UI流程
                    self.master.after(0, self.setup_ui)
    ...
    def hide_window(self):
        ...
        def setup_system_tray():
            ...
            self.icon = pystray.Icon("campus_net_login",
                                      icon=icon_image,
                                      title="校园网自动登录",
                                      menu=pystray.Menu(item('打开', self.show_window, default=True),
    ...
    def quit_app(self, icon, item=None):
        ...
        self.master.after(0, quit_app_main_thread)  # 使用after方法来安排在主线程中执行退出应用的操作
    ...
    def open_suggestion_box(self):
        ...
        tk.Label(suggestion_window, text="请分享您的建议或反馈：").pack(padx=10, pady=5)  # 在窗口中添加文本标签
        suggestion_text = tk.Text(suggestion_window, height=10, width=50)  # 创建一个文本框用于输入建议
        suggestion_text.pack(padx=10, pady=5)  # 将文本框放置在窗口中

        def submit_suggestion():
            suggestion_content = suggestion_text.get("1.0", "end").strip()  # 获取用户输入的建议并去除首尾空格
            if suggestion_content:
                self.save_suggestion(suggestion_content)  # 调用save_suggestion方法保存建议
                messagebox.showinfo("提交建议", "您的建议已提交，感谢您的反馈！")  # 弹出信息提示框，确认建议已提交
                suggestion_window.destroy()  # 销毁提交建议窗口
            else:
                messagebox.showwarning("提交建议", "建议内容不能为空。")  # 弹出警告提示框，提醒建议内容不能为空

        tk.Button(suggestion_window, text="提交", command=submit_suggestion).pack(pady=5)  # 在窗口中添加提交按钮，并设置点击事件为submit_suggestion函数
    ...
    def setup_ui(self):
        ...
        self.center_window(326, 286)  # 调用center_window方法将窗口居中，并设置窗口大小为326x236

        ttk.Label(self.master, text="用户名：").grid(row=0, column=0, padx=30, pady=20, sticky='w')  # 创建用户名标签并设置位置和对齐方式
        self.username_entry = ttk.Entry(self.master)  # 创建用户名输入框
        self.username_entry.grid(row=0, column=1, padx=3, pady=5, sticky='ew')  # 设置用户名输入框位置和对齐方式

        ttk.Label(self.master, text="密码：").grid(row=1, column=0, padx=30, pady=0, sticky='w')  # 创建密码标签并设置位置和对齐方式
        self.password_entry = ttk.Entry(self.master, show="*")  # 创建密码输入框，显示为*
        self.password_entry.grid(row=1, column=1, padx=3, pady=5, sticky='ew')  # 设置密码输入框位置和对齐方式

        self.password_strength_label = ttk.Label(self.master, text="")  # 创建用于显示密码强度的标签
        self.password_strength_label.grid(row=2, column=1, padx=10, sticky='w')  # 设置密码强度标签位置

        # 密码可见性切换按钮
        self.toggle_password_btn = tk.Button(self.master, image=self.eye_closed_icon, command=self.toggle_password_visibility, borderwidth=0)  # 创建密码可见性切换按钮
        self.toggle_password_btn.grid(row=1, column=2, sticky='e')  # 设置密码可见性切换按钮位置

        self.remember_var = tk.IntVar()  # 创建用于记住账号和密码的变量
        ttk.Checkbutton(self.master, text="记住账号和密码", variable=self.remember_var).grid(row=3, column=0, columnspan=1, padx=20, pady=5)  # 创建勾选框
        ...
        ttk.Button(self.master, text="报告问题", command=self.report_error).grid(row=6, column=0, padx=30, pady=10, sticky='ew')  # 创建报告问题按钮
        ttk.Button(self.master, text="提交建议", command=self.open_suggestion_box).grid(row=6, column=1, padx=6, pady=10, sticky='ew')  # 创建提交建议按钮

        # 自动填充逻辑
        username, password, isp, remember = self.load_credentials()  # 加载保存的凭据
        if username and password and remember:  # 如果有保存的用户名、密码，并且选择了记住
    ...
    def open_settings(self):
        ...
        tk.Checkbutton(settings_window, text="开机时自动启动", variable=self.auto_start_var).grid(row=2, column=1, columnspan=2, padx=10, pady=10, sticky='w')

        # 在设置窗口中添加“自动登录”复选框
        self.auto_login_var = tk.IntVar(value=self.config.get("auto_login", False))
        auto_login_checkbox = tk.Checkbutton(settings_window, text="自动登录", variable=self.auto_login_var)
        auto_login_checkbox.grid(row=1, column=1, columnspan=2, padx=10, pady=10, sticky='w')

        # 创建API URL输入框
        tk.Label(settings_window, text="API URL:").grid(row=0, column=0, padx=10, pady=10, sticky='e')
        ...
        tk.Button(buttons_frame, text="保存", command=save_settings_wrapper).pack(side=tk.LEFT, padx=5)
        tk.Button(buttons_frame, text="取消", command=lambda: self.on_settings_close(settings_window)).pack(side=tk.RIGHT, padx=5)
    ...
    def clear_key_and_credentials(self):
        ...
        confirm = messagebox.askyesno("确认清除", "这将清除所有保存的用户凭证和加密密钥。您确定要继续吗?")
        if confirm:
            key_cleared = False
            ...
            if key_cleared and credentials_cleared:
                ...
                messagebox.showinfo("清除失败", "未找到加密密钥和用户凭证，无需进行清除。")
    ...
    def clear_credentials(self):
        """仅清除存储的用户凭证"""
        # 弹出确认对话框，让用户确认清除操作
        confirm = messagebox.askyesno("确认清除", "这将清除所有保存的用户凭证。您确定要继续吗？")
        if confirm:
            try:
                # 尝试删除存储用户凭证的文件
                os.remove('credentials.pkl')
    ...
    def apply_auto_start_setting(self):
        ...
        shortcut_path = os.path.join(start_up_folder, 'CampusNetLoginApp.lnk')  # 设置快捷方式路径和文件名
        if self.config.get("auto_start"):  # 检查配置中是否开启了自动启动
            if not os.path.exists(shortcut_path):  # 如果快捷方式不存在
                # 使用winshell创建快捷方式
                script_path = os.path.join(os.getcwd(), '校园网.exe')  # 设置可执行文件的路径(假设文件名为校园网.exe)
    ...
    def show_notification(self, title, msg, icon_path=None):
        ...
        wc.lpfnWndProc = {win32con.WM_DESTROY: self.on_destroy}  # 可以添加更多消息处理。
        ...
        self.hwnd = win32gui.CreateWindow("CampusNetLoginAppNotification", "CampusNetLoginApp Notification Window", style,
        ...
        # 显示通知。
        if icon_path and os.path.isfile(icon_path):  # 检查是否有指定图标路径并且文件存在
            icon_flags = win32con.LR_LOADFROMFILE | win32con.LR_DEFAULTSIZE
            hicon = win32gui.LoadImage(None, icon_path, win32con.IMAGE_ICON, 0, 0, icon_flags)  # 加载图标
        else:
        ...
        win32gui.Shell_NotifyIcon(win32gui.NIM_MODIFY, 
                                  (self.hwnd, 0, win32gui.NIF_INFO, win32con.WM_USER+20,
                                   hicon, "Balloon Tooltip", msg, 200, title))  # 修改通知为气球提示
...
if __name__ == "__main__":
    ...
    root.protocol("WM_DELETE_WINDOW", lambda: on_main_close(root))  # 设置关闭窗口时的操作为调用on_main_close函数

    config = load_or_create_config()  # 调用load_or_create_config函数来加载或创建配置文件
    show_ui = not config.get("auto_login", False)  # 根据配置文件中的auto_login字段确定是否显示UI界面
