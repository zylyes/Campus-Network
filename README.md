...
| V1.4.2 | 2024-05-04 | **新增功能：**<br>  - 调整主界面创建方式，使之能更好地适配各种显示条件。 | [下载](https://github.com/zylyes/Campus-Network/releases/tag/V1.4.2) |
| V1.4.3 |   计划中   | **新增功能：**<br>  - 将与 GUI 不相关的功能进一步抽象和封装到类中，<br>提升代码可维护性和扩展性。 | [待定](https://github.com/zylyes/Campus-Network/releases/tag/V1.4.1) |

## 未来计划：

1. **界面优化和适配**：将界面由像素界面改为图片界面，并且支持动态调整分辨率以适配不同的显示屏和显示倍率。计划在下一个版本中实现，预计完成时间为 2024 年 5 月。优先级：中。（已完成部分）
...
1.通过点击版本更新记录内的链接或者点击[发行版](https://github.com/zylyes/Campus-Network/releases)跳转到下载界面。

2.选择自己需要的版本下载。

**安装**

在下载完成后双击运行安装程序，按照程序指引完成安装。

**运行**

找到安装目录所在，双击运行应用程序即可（如创建了桌面快捷方式，则直接双击运行）
...
1. **连接校园网络**：
 ...
 - 按照界面提示，输入校园账号和密码，点击连接按钮。

 - 根据个人需要决定是否勾选记住和保存密码（自动登录依赖记住密码）。

2. **设置配置**：

 - 在主界面中点击设置按钮，即可打开设置界面。

 - 管理员用户可以对设备进行管理，如禁用设备、重新连接等操作。
